...
SELECT 
    ...
    p.is_active,
    COALESCE(SUM(is.quantity), 0) as total_stock
FROM products p
...
GROUP BY 
    p.id, p.sku, b.name, pt.name, pt.description, 
    p.base_price, c.category_code, ct.name, l.code, p.is_active;
...
JOIN product_translations pt ON p.id = pt.product_id
JOIN measurement_units mu ON is.unit_id = mu.id
JOIN languages l ON pt.language_id = l.id;
